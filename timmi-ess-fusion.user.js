...
// ==/UserScript==
  ...
  (async()=>{
                ...
                function makeModernEssCompact() {
                    ...
                    $('.jss34').css('margin-left', 'unset').css('max-width', 'unset');
                ...
                function calculateDiffForDay(element) {
                    ...
                    var sameDayFields = $('input').filter(function() {
                        return isWorkdayInputField($(this)) && $(this).attr('id').endsWith(idSuffix);
                    });
                ...
                function fixWbsOverviewLayout() {
                    ...
                    $('.urTbsDiv').css('height', '1%');
                    // firefox sizing issue
                    $('.urTbsDiv').css('background-color', 'white');
                    // firefox coloring
                    ...
                    // unfold/fold button sizing issue
                    ...
                    $('.urBtnPaddingEmph').css('margin', '10px').css('padding', '5px');
                    $('.urBtnPrevPadding').css('padding', '5px').text('Back');
                ...
                function isWorkdayInputField(element) {
                    ...
                    return element.attr('id').indexOf('.mondhours') !== -1 || element.attr('id').indexOf('.tueshours') !== -1 || element.attr('id').indexOf('.wedhours') !== -1 || element.attr('id').indexOf('.thurhours') !== -1 || element.attr('id').indexOf('.fridhours') !== -1 || element.attr('id').indexOf('.sathours') !== -1 || element.attr('id').indexOf('.sunhours') !== -1;
                }
                ...
                async function appendEss() {
                    ...
                    loginBtn.css('position', 'fixed').css('bottom', '15px').css('right', '20px').css('width', '80px').css('height', '20px').css('z-index', '100').css('padding', '0');

                    var oldLoginBtn = $('<button id="oldLoginBtn">old ESS</button>');
                    $('body').append(oldLoginBtn);
                    setButtonStyle(oldLoginBtn);
                    oldLoginBtn.css('position', 'fixed').css('bottom', '15px').css('right', '120px').css('width', '80px').css('height', '20px').css('z-index', '100').css('padding', '0').css('background-color', 'lightgray').css('color', 'black');
                ...
                function appendHelpBox() {
                    ...
                    $('body').append(helpBtn);
                    helpBtn.css('position', 'fixed').css('bottom', '17px').css('right', '220px').css('z-index', '100').css('font-family', 'arial').css('font-size', '14px').css('text-decoration', 'none');
        ...
        <br>
                    ...
                    $('body').append(helpBox);
                    helpBox.css('background-color', 'white').css('border', '1px solid gray').css('position', 'fixed').css('bottom', '50px').css('right', '220px').css('width', '550px').css('height', '200px').css('z-index', 1000).css('font-family', 'arial').css('font-size', '14px').css('padding', '10px');
                    helpBox.hide();
                ...
                function setButtonStyle(btn) {
                    ...
                    btn.css('display', 'inline-block').css('text-decoration', 'none').css('background-color', '#66A3C7').css('border', '1px solid gray').css('color', 'white').css('font-size', '10px').css('font-family', 'arial').css('white-space', 'nowrap').css('padding', '5px').css('border-radius', '2px').css('veritcal-align', 'middle').css('text-align', 'center').css('border-radius', '5px').css('cursor', 'pointer');
                }

                function minimizeEssFrame() {

                    var essIframe = $('#essIframe');
                    essIframe.attr('maximized', 'false').css('z-index', '100').css('border', '1px solid lightgray').css('border-radius', '10px').css('width', '100%').css('height', '600px');
                ...
                function maximizeEssFrame() {
                    ...
                    var essIframe = $('#essIframe');
                    essIframe.attr('maximized', 'true').css('top', '50px').css('left', '50px').css('width', 'calc(100% - 55px)').css('height', 'calc(100% - 55px)').css('border', '1px solid gray');
                }
                ...
                function isSessionTimedOut() {
                    ...
                    return ($('form[name="loginForm"]').length > 0 || $('*:contains("Session not found")').length > 0 || $('*:contains("Session does not exist")').length > 0 || $('*:contains("Session timed out")').length > 0 || $('*:contains("session is expired")').length > 0 || $('*:contains("This logon is obsolete and deprecated")').length > 0 || $('*:contains("Choose one of the available Identity Providers")').length > 0 || $('*:contains("Service Not Available")').length > 0 || $('*:contains("You logged of")').length > 0);
                }
                ...
                function fixLayout() {
                    ...
                    // current week box
                    $('input[name="timesheet_pperiod"]').css('height', '24px').css('width', '55px').css('text-align', 'center').css('border-radius', '5px');

                    // remove "Hours" text
                    $('span:contains("Hours")').filter(function() {
                        return $(this).text() == "Hours"
                    }).text('');

                    // fix week total width
                    var timeCells = $('input[id*="hours"],input[id*=".weektotal"]')
                    timeCells.css('padding', '5px').css('font-size', '12px').css('width', '47px').css('text-align', 'right');
                    ...
                    // add saferpay Logo
                    $('#htmlb_image_1').attr('src', _saferpayLogo).css('margin-right', '10px').wrap('<a href="https://www.saferpay.com" target="_blank"></a>');

                    $('.urGrpTtlBox').css('padding', '5px').css('border-radius', '5px').css('background-color', 'lightblue');
                    // #5FC7A9

                    // border for legend;
                    $('img[id^="htmlb_image_"]').css('border-radius', '5px');
                    ...
                    $('input[value="0,00"]').filter(function() {
                        return $(this).attr('id').indexOf('timesheet_tsdurationdata[2].') == -1;
                    }).css('color', 'lightgray');
                        ...
                        timmiRow.find('input[id*="hours"]').each(async function(dayOfWeek) {
                            ...
                            if (currentDay.format('YYYY-MM-DD') == e.originalEvent.data.day) {
                                ...
                                if (parseFloat(Math.abs(diff.toFixed(2))) !== 0) {
                                    ...
                                    diffCell.css('background-color', 'lightcoral').css('color', 'white');
                    ...
                    $(window).on('message', function(e) {
                        ...
                        $('footer span:contains("/")').each(async function(dayOfWeek) {
                            ...
                            if (currentDay.format('YYYY-MM-DD') == e.originalEvent.data.day) {
                                ...
                                var diffCell = $(this);
                                diffCell.val('').css('background-color', '').css('color', '#6D6D6D').css('padding', '4px').css('border-radius', '0 4px 4px 0');

                                diffCell.prev().css('background-color', '').css('color', '#6D6D6D').css('padding', '4px').css('border-radius', '4px 0 0 4px');
                                ...
                                if (parseFloat(Math.abs(diff.toFixed(decimalPlaces))) > Number.EPSILON) {
                                    diffCell.css('background-color', 'lightcoral').css('color', 'white');
                                    diffCell.prev().css('background-color', 'lightcoral').css('color', 'white');
                                    ...
                                    diffCell.css('background-color', '#c7ebe8').css('color', '#00A59C');
                                    diffCell.prev().css('background-color', '#c7ebe8').css('color', '#00A59C');
                                }
                            }
                        });
                    });
                ...
                function addEndOfMonthWarningModernEss() {
                    ...
                    if ($('#monthEndWarning').length == 0) {
                        var text = secondLastWorkday.format('dddd DD.MM.YYYY') + ' is the second last working day of ' + secondLastWorkday.format('MMMM') + '. You should fill out and release the whole month no later than ' + secondLastWorkday.format('dddd') + '.';
                        $('#mainHeader').parent().parent().append($('<span style="background-color:lightyellow;padding:5px;font-size:12px;border-radius:5px;" id="monthEndWarning">' + text + '</span>')).removeClass('MuiGrid-grid-xs-6').addClass('MuiGrid-grid-xs-11').next().removeClass('MuiGrid-grid-xs-6').addClass('MuiGrid-grid-xs-1');
                    }
                ...
                function addFillButtonsModernEss() {
                    ...
                    $('input.MuiInputBase-input:not([readonly])').each(function() {
                        ...
                        fillDayBtn.hide();
                        // avoid showing buttons from other month
                    ...
                    btn.click(function() {
                        if (fillCellWithDayDiffModernEss($(this).next().find('input'))) {
                            $('button>span:contains("Save")').focus().click();
                        }
                    });
                ...
                function formatDayTitles() {
                    ...
                    $('#dateRow').find('.urTxtStd:contains("/")').each(function() {
                        ...
                        var newText = $(this).text().replace('/', '.').replace('MO', 'Monday<br>').replace('TU', 'Tuesday<br>').replace('DI', 'Tuesday<br>').replace('WE', 'Wednesday<br>').replace('MI', 'Wednesday<br>').replace('TH', 'Thursday<br>').replace('DO', 'Thursday<br>').replace('FR', 'Friday<br>').replace('SA', 'Saturday<br>').replace('SU', 'Sunday<br>').replace('SO', 'Sunday<br>');

                        newText += '.' + new Date().getFullYear();
                        if (newText.indexOf(moment().format('DD.MM.YYYY')) != -1) {
                            $(this).css('font-weight', 'bold');
                            newText = newText.replace('Monday', 'Today').replace('Tuesday', 'Today').replace('Wednesday', 'Today').replace('Thursday', 'Today').replace('Friday', 'Today').replace('Saturday', 'Today').replace('Sunday', 'Today')
                ...
                function hideWeekend() {
                    ...
                    var elements = [$('input[id*=".sathours"]').closest('td'), $('input[id*=".sunhours"]').closest('td'), $('span[weekTitle]').eq(5).closest('td'), $('span[weekTitle]').eq(5).closest('td')];
